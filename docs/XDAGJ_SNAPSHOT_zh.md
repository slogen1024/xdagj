...
此外，`--makesnapshot` 若打印 `chain state snapshot NOT written: …`，说明区块/地址快照已经写好、但 chain 快照没写成（最常见原因是 `SNAPSHOT/CHAIN_L1` 已存在且非空——重复导出会与旧内容混在一起，因此被拒绝）。此时高度与 next start frame 仍会照常打印，但**这份快照不能用于启动激活高度及以后的节点**：删除已存在的 `SNAPSHOT/CHAIN_L1` 后重新导出。
