...
- **P3 (Medium, miner griefing):** a *validation* failure (nonce mismatch, insufficient balance) is written as a status-0 / gasUsed-0 receipt and the tx hash is thereafter deduplicated away, while the sender nonce is unchanged — any miner can "burn" any pending tx at zero cost by referencing it out of order. Report's suggested fix (not yet decided): validation failures stop consuming the hash, or are dropped from the execution list; either is deterministic and fork-gated.
- **B1 (Medium, δ ≥ 2):** between `setMain(M + δ − 1)` and an async blob-drain, a behind node has not yet credited block M's fee, so it rejects the pool's payout block spending that amount (insufficient balance → `BI_MAIN_REF` only) and never re-evaluates it; the withdrawal path freezes CRITICAL in the same situation, the fee path diverges silently. Report's suggested fix (not yet decided): align with the release path (CRITICAL + re-sync) or do not credit heights executed via async drain.
- **B2 (Low):** the burn scan does not require receipt `status == 1`; the only path that leaves logs on a halted frame is the S-26 `catch (RuntimeException)` in `XdagEvmExecutor.runToHalt` (reachability unproven).
- **E1–E6 (deterministic semantics, all nodes agree, contracts misbehave):** `SELFDESTRUCT` never deletes the account (no `getSelfDestructs` consumer; balance cleared, code/storage/nonce kept); `BASEFEE` **exceptional-halts** (Besu `BaseFeeOperation` with empty base fee — the freeze-1 brief wrongly said "reads 0"), `GASPRICE` reads 0 while the sender pays `effectiveGasPrice`, `BLOCKHASH` reads 0; `eth_call` / `eth_estimateGas` simulate with an all-zero block context (number/timestamp 0); EIP-170 (24 KiB code) and EIP-3541 (`0xEF` prefix) are not enforced (empty contract-creation rule list); nested-frame `getOriginalStorageValue` returns the parent's *current* value; consensus parameters (`minGasPrice`, `blockGasLimit`, `stateRootLag`, `bridgeWithdrawalDelay`, `alloc`, recovery address) are node-local HOCON rather than pinned per network. All fixes change receipts/roots and will be fork-gated.
- **R1–R7 (RPC returns wrong data; no auth/injection issue):** block-tag / `eth_blockNumber` / `eth_getLogs` use the *native* head while receipts land δ − 1 blocks later (indexers miss logs at δ ≥ 2); `eth_getLogs` ignores the `blockHash` filter; `"pending"` nonce ignores the pool; receipt `logIndex` is per-tx and `cumulativeGasUsed == gasUsed`; `eth_estimateGas` is a single run without the 63/64 margin; `logsBloom` is always zero; no JSON-RPC batch, revert data only in `message`, `newHeads` placeholder headers, WS upgrade requires an `Authorization` header.
...
- Opcode context is deliberately minimal: `GASPRICE`, `BLOCKHASH`, `PREVRANDAO`, `COINBASE` read 0 (deterministic, no randomness claim); `BASEFEE` currently exceptional-halts (open item E2, §6) — contracts relying on it fail deterministically.
