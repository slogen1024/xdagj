...
- **State commitment:** the EVM state is committed as a **chained delta root** (a running keccak chain over per-height execution digests), *not* an account MPT — there is no `eth_getProof`. The root is PoW-committed: each main block carries an `EvmStateAnchor` field `(height, root(H−δ−1), daSkip)` (index corrected 2026-09-05 after internal audit round 2, finding C3; the audited tag `evm-audit-freeze-1` still carries the unsatisfiable `root(H−δ)` rule); a mismatching anchor is rejected at import and hard-rejected in `setMain` on mainnet configuration (warn-only on testnet/devnet).
