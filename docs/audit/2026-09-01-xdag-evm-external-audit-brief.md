...
**Date:** 2026-09-08 (revision 3; first issued 2026-09-01) · **Audited ref:** tag `evm-audit-freeze-3` = commit `e4c6fe46`, branch `dev-evm`, repo `https://github.com/slogen1024/xdagj` (supersedes `evm-audit-freeze-2` = `a94d47e2` and `evm-audit-freeze-1` = `3bbfc68f`; see §6 for the deltas) · **Status of this document:** engagement entry point — self-contained; no other project document is required to scope the engagement. This file lives on `dev-evm` in a docs-only commit after the tag; the code under audit is exactly the tag.

## 1. Engagement summary

XDAGJ is the Java implementation of XDAG, a DAG-based PoW cryptocurrency (RandomX). The `dev-evm` branch embeds a **Hyperledger Besu-EVM execution layer** into the node: EVM transactions ride the existing DAG transport, execute deterministically at main-block confirmation, and interoperate with native XDAG through a two-way bridge. The work is feature-complete and has had two internal audit rounds (2026-08-30, 2026-09-04), both fully closed at this tag; **no shared network runs it yet** — on testnet/mainnet configuration every activation height is `Long.MAX_VALUE` and `evm.enabled = false`; only the throwaway devnet is live.

We are seeking a security audit of the EVM integration ahead of scheduling any activation on a public network.

- **Audited surface:** `io.xdag.evm.*` (26 files, ~5,500 lines) plus its integration points: four consensus hooks in `io.xdag.core.BlockchainImpl` (§2), the versioned block codec in `io.xdag.core.Block` / `EvmStateAnchor`, five P2P message codes in `io.xdag.net`, the `eth_*` JSON-RPC / WebSocket handlers in `io.xdag.rpc`, and the `evm.*` configuration surface in `io.xdag.config` (with the per-network consensus-parameter pins). Full branch delta vs `develop`: 248 files, +40,112 / −371 lines (measured at the audited tag).
- **Verification baseline at the audited ref:** `mvn test` = 703 tests, 0 failures, 0 errors, 0 skipped; `mvn license:check` clean (JDK 21).
...
- **Execution semantics v2 (fork pack, `evm.semanticsV2ActivationHeight`):** one gate switches on the round-2 execution fixes — SELFDESTRUCT deletes the account (+ EIP-161 touched-empty cleanup) at the transaction epilogue, `BASEFEE` reads 0, `GASPRICE` reads the effective price, `BLOCKHASH` resolves canonical main-block hashes, EIP-170 / EIP-3541 creation rules, transaction-start "original" storage in nested frames, and a failed frame never surfaces logs (so the bridge burn scan reads only successful receipts). Below the gate every path is byte-identical to the pre-fork behaviour. A second gate, `evm.invalidTxSkipActivationHeight`, drops validation-failed refs (nonce mismatch, unaffordable, undecodable, …) from a block instead of recording a status-0 receipt that consumed the hash forever.
- **Pinned consensus parameters:** on testnet/mainnet every parameter that decides receipts, budgets or the chained root (chain id, block gas limit, min gas price, state-root lag, withdrawal delay, recovery address, genesis alloc, every fork height) is pinned in code (`EvmConsensusParams`); a node whose loaded configuration differs refuses to start. Devnet stays configuration-driven.
- **Executed-head RPC:** every `eth_*` block tag, `eth_blockNumber`, log range and `newHeads` push resolves against the highest EVM height the node has fully EXECUTED (`Blockchain.getEvmExecutedHeight`), which trails the native head by up to δ − 1 plus any blob-deferred height — so receipts, logs, tx lists and state are consistent for a given block number.
...
### P1-9 · Execution semantics v2 pack (`evm.semanticsV2ActivationHeight`)
Is the fork gate applied at every site (creation processor selection, transaction epilogue, `collect()` log suppression, block values' base fee, the E5 updater flag, the burn-scan filter) so that below the gate execution is byte-identical to freeze-2 and above it identical on every node? Is the SELFDESTRUCT / EIP-161 epilogue (`XdagEvmExecutor.finishTransaction`, run on the tx-level updater before commit) complete — nested self-destructs merged up, the beneficiary transfer kept, a re-created address (CREATE2) starting clean — and can it delete an account it must not (sender, bridge contract, an account touched but non-empty)? Is the `BLOCKHASH` lookup canonical under reorg and replay (heights ≤ the replay target only)? Does the E5 "original storage" climb return the transaction-start value at every frame depth, including after an inner frame committed into its parent? Can the invalid-tx-skip gate be abused to make a miner's block cheaper to grief with (dropped refs cost the block nothing)?
→ `io.xdag.evm.XdagEvmExecutor` (ExecutionOptions / finishTransaction / collect), `io.xdag.evm.EvmBlockProcessor` (executeOne / executeList / burnScanEligible / zeroGasCall), `io.xdag.evm.state.RocksDbAccount` (getOriginalStorageValue), `io.xdag.Kernel` (block-hash lookup wiring).

### P2-9 · RPC surface
`eth_call` / `eth_estimateGas` can never commit state (simulation path fully separated from consensus path)? Bounded `eth_getLogs` (scan-range cap)? Auth constant-time, CORS split sane, and the WebSocket `?token=` query parameter not weaker than the header (never logged, never echoed)? (Loopback bind is the shipped default.) Oversized transactions rejected at `eth_sendRawTransaction` (−32602)? Executed-head semantics: is `getEvmExecutedHeight` always ≥ the highest checkpoint, and can a client ever be served a block number whose receipts/logs/state are not yet consistent? Batch requests: per-entry isolation, the 100-call cap, and no amplification through nested work (`eth_estimateGas` bisection × batch)?
...
Full-pool priority eviction (price ASC → sender-load DESC → age ASC; tails only; incoming sender excluded): can it be weaponized for targeted eviction? Nonce-contiguity preserved? Does `nextNonce` (the "pending" tag) ever hand out a nonce that cannot be admitted?
→ `io.xdag.evm.tx.EvmTxPool`.

### P2-11 · Pinned consensus parameters (`EvmConsensusParams`)
Do the TESTNET / MAINNET constants match what the shipped HOCON files declare (the loader refuses a mismatch, so a drift here is a start-up failure, never a fork)? Is every consensus-relevant key covered — is there any `evm.*` value that still reaches execution unpinned? Are the sanity checks (lag ≥ 1, gas limit > 0, chain id > 0, min gas price ≥ 0, withdrawal delay ≥ 1) sufficient to keep `setMain` from throwing on a bad value?
→ `io.xdag.config.EvmConsensusParams`, `io.xdag.config.AbstractConfig` (enforcePinnedEvmConsensusParams / validateEvmConsensusSanity), `TestnetConfig` / `MainnetConfig`.
...
15. From `evm.invalidTxSkipActivationHeight`, a validation failure touches no state, charges no gas and leaves no receipt: the tx hash stays executable in a later block, so no miner can burn a pending tx by referencing it out of order.
16. From `evm.semanticsV2ActivationHeight`, a self-destructed account and every touched-empty account are gone at transaction end; a failed frame surfaces no logs; below the gate both behaviours are exactly the audited freeze-2 ones.
17. On a shared network the loaded consensus parameters equal the pinned constants or the node does not start; a consensus parameter change is a release, never a setting.
18. Every `eth_*` block number the node reports is at or below its executed EVM head, and at that number receipts, logs, tx lists and state are mutually consistent.
...
### 6.3 Round 2 remainder — the freeze-2 → freeze-3 delta

Every remaining round-2 finding is fixed at freeze-3 (each on the `fix/evm-audit-round2-remaining` branch, TDD, merged `--no-ff`; P3/B1 on their own branch just before it). The pre-fix behaviour is pinned by tests as "legacy" wherever a fork gate applies. **Please re-examine these areas with the review asks noted.**

| ID | Severity (at freeze-2) | Finding | Fix at freeze-3 (commit) |
|----|------------------------|---------|--------------------------|
| P3 | Medium (miner griefing) | A *validation* failure (nonce mismatch, unaffordable, …) was recorded as a status-0 receipt and the tx hash deduplicated away while the sender nonce stayed put — any miner could burn any pending tx at zero cost | Dropped from the block outright (no receipt, no digest entry, budget returned) from `evm.invalidTxSkipActivationHeight`; legacy below (acafc3bd). *Ask:* determinism of the validation verdict, grief cost of dropped refs |
| B1 | Medium (δ ≥ 2) | A blob-behind node had not credited block M's fee, rejected the pool payout spending it and never re-evaluated it — a silent native fork where the withdrawal path freezes CRITICAL | `applyBlock` detects a rejected spend from a still-deferred payload block, records `evmFeeDivergenceHeight`, logs CRITICAL with re-sync guidance; deferral and late credit logged; K1 drain credit kept (cca699e0). *Ask:* is loud-and-re-sync sufficient, or should the credit be made blob-independent? |
| E1/E2/E4/E5/B2 | Medium–Low (deterministic semantics) | SELFDESTRUCT never deleted; `BASEFEE` halted, `GASPRICE`/`BLOCKHASH` read 0; no EIP-170/3541; nested-frame "original" storage wrong; burn scan read failed receipts | One fork pack behind `evm.semanticsV2ActivationHeight` (196f8d3c); see P1-9. *Ask:* gate completeness, the deletion epilogue |
| E3 | Medium (functional) | `eth_call` / `eth_estimateGas` simulated in an all-zero block context | Simulation runs in the tagged block's context with the fork options in force there; intrinsic gas charged first (bd1e8013) |
| E6 | Low (design) | Consensus parameters were node-local HOCON | `EvmConsensusParams` pins per shared network, mismatch refuses to start, sanity checks everywhere (9366265c); see P2-11 |
| R1–R6 | Medium–Low (wrong data) | Native-head block tags (indexers missed logs at δ ≥ 2), no `blockHash` filter, pool-blind "pending" nonce, per-tx `logIndex`, single-run `estimateGas`, zero blooms | Executed-head resolution, EIP-234 `blockHash`, `nextNonce`, block-wide receipt coordinates, bisected `estimateGas`, real blooms (bd1e8013) |
| R7 | Feature gaps | No batches; revert data only in `message`; placeholder `newHeads`; WS needed an `Authorization` header | Batches over HTTP + WS (cap 100, per-entry isolation); code-3 reverts with `data`; real `newHeads` headers on the executed head; WS `?token=` (3027b066). *Ask:* batch DoS surface |

**Open at freeze-3 (acknowledged, non-blocking for the review):** operator recovery runbook for corrupt EVM_META and for the ADDRESS-CF genesis-lock marker on snapshot bootstrap (fail-fast today); batch decode-loop budget bound; optional inbound rate-limit / HTTP `Host`-header allowlist (mitigated by the loopback default); shared-net economic values (`blockGasLimit`, `minGasPrice`) are framework placeholders pinned as such until reviewed; bridge tooling (ABI publication, remark encoder, a visible release record).
...
git checkout evm-audit-freeze-3
mvn clean package -DskipTests     # build
mvn test                          # expected: 703 tests, 0 failures, 0 errors, 0 skipped
...
- We will fix and tag `evm-audit-freeze-4`; one fix-verification round against that tag is in scope.
...
**Revision history.** r1 2026-09-01: issued against `evm-audit-freeze-1` (3bbfc68f, 633 tests). r3 2026-09-08: re-issued against `evm-audit-freeze-3` (e4c6fe46, 703 tests) — round 2 fully closed; §2 gains the semantics-v2 pack, pinned parameters and executed-head RPC; new surfaces P1-9 / P2-11; invariants 15–18; §6.3 tabulates the freeze-2 → freeze-3 delta with review asks; open items re-listed. r2.2 2026-09-08: §6.2 notes E1–E6, B2 and R1–R7 as fixed on `dev-evm` after the tag (196f8d3c / 9366265c / bd1e8013 / 3027b066); §7 opcode limitation split by the semantics-v2 gate. r2.1 2026-09-08: §6.2 notes P3 and B1 as fixed on `dev-evm` after the tag (acafc3bd / cca699e0, suite 670). r2 2026-09-07: re-issued against `evm-audit-freeze-2` (a94d47e2, 665 tests) — anchor rule corrected to `root(N − δ − 1)`, import-time rejection, δ-lag re-open, content-classified ingest, admission size cap, consensus-frozen amount conversion, lenient anchor parse; §6.2 added with the round-2 delta and the open items; `BASEFEE` limitation corrected; priority surfaces P1-8 added, P2-8/P2-9 renumbered to P2-9/P2-10.
