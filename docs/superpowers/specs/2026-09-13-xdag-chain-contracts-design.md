# XDAG 链式 DAG 原生智能合约总设计（Chain Contracts）
...
**方法**：把"链（Chain）"作为 DAG 原生的状态分区。

- **L1（现有 XDAG DAG）只做四件事**：排序（PoW 主链 × `applyBlock` DFS）、数据可用性（所有链块都是 512B DAG 块，`NO_PARENT` 导入规则天然强制 DA）、原生账本与金库、仲裁（单步 WASM 重执行）。
- **链**承载 1..N 个 WASM 合约，状态 = L1 输入流的纯函数（based 排序，执行者无法重排或审查）。链内同步原子可组合；链间异步消息，消息就是链 outbox 里的条目，交付延迟 D 由目标链自配。
- **执行者**只跑自己订阅的链。**认证者**锁定 XDAG 保证金后发布锚定块（状态根 + 逐输入承诺）。**任何人**都可用"单步重执行 + 状态见证"的欺诈证明挑战错误锚定并获得罚没奖励。
...
| G2 | 放弃 EVM 兼容 | 无 Besu 依赖、无 eth_* RPC、无 keccak 地址空间；用户在链内与 L1 使用同一地址 |
| G3 | 执行/状态吞吐无上限 | 节点只执行/存储订阅链；新增链与执行者线性扩容；核数扩展曲线近线性 |
| G4 | 安全不弱于 L1 | 排序与 DA 由 PoW 保证；链正确性由"1 个诚实观察者 + 挑战窗口"保证；金库无无背书流出 |
| G5 | 通用 DApp 基础 | 同链内 EVM 级同步可组合；跨链异步可组合；参考 DApp：代币、DEX、跨链演示 |
...
| # | 决策 | 选项与理由 |
|---|------|-----------|
| D1 | **分片执行一步到位** | 备选"先全复制并行、后分片"被否。v1 即包含链订阅、保证金认证者与欺诈证明 |
...
| D4 | **开放保证金认证者 + 无状态欺诈证明** | 备选"运营方指定执行者"（去中心化弱）、"链级合并挖矿"（冷门链算力不足）。based 排序使认证者只是"结果担保人"，任何人可接管 |
| D5 | **L1 保留原生账本 + 链金库** | 备选"原生账本迁入系统链"（需迁移全部存量余额与钱包/交易所集成，风险最大） |
| D6 | **`subscribeAll` 过渡模式纳入 v1** | 节点可配置订阅全部链，退化为全复制并行执行，降低一步到位的交付风险（§20.3 D2） |
...
| 项目 | 与本设计的关系 | 采纳 | 不采纳 |
|------|---------------|------|--------|
| **Kaspa**（唯一同类 PoW BlockDAG）| L1 极简（covenants + zk 验证），执行交给 based rollup；L1 只做排序与 DA | based 排序、L1 不执行通用逻辑 | zk（Java 生态不可行）；应用各自搭 L2（我们把 VM 与仲裁"内置"，任何节点可跟任何链） |
| **Linera**（微链）| 同一验证者集并行执行百万条链；链内同步可组合、链间异步消息 | 链 = 微链的 XDAG 化；链内同步 + 链间异步 | 需要验证者身份（我们用保证金 + 欺诈证明替代） |
| **NEAR Nightshade** | 异步分片、收据式跨分片调用、无状态验证（状态见证） | 收据即输入、状态见证做单步验证 | 出块者/验证者分配（PoS） |
| **Sui / Aptos** | 对象所有权快路径、Block-STM 乐观并行 | "链内独立即并行"的思想 | 单一世界状态的块内并行（不解决横向扩展） |
| **Vite**（XDAG 最近亲：DAG + 快照链 + 异步合约）| 2019 上线后式微：**纯异步编程模型对开发者过于痛苦** | 快照链 = 主链的排序作用 | 纯异步（我们保留链内同步） |
| **CosmWasm** | actor 模型结构性防重入、submessage reply | 默认禁止重入 | 完全禁止同步调用（DEX 需要同链原子撮合） |
| **Kadena Chainweb** | PoW 编织平行链、SPV 跨链 | 编织式跨链原子步（v2） | 固定链数 |
...
**结论**：XDAG 已有的"主链确认序 × DFS 序"是天然的 based 排序器；512B 块与 `NO_PARENT` 规则是天然的 DA 层；缺的只是（a）分区的执行/状态模型，（b）不依赖验证者身份的结果担保机制。本设计用"链 + 保证金认证者 + 单步欺诈证明"补上这两块。
...
| 排序 | 主块每 64s（epoch）一个；确认 ≈ 2 epoch；`setMain` → `applyBlock` 递归 DFS，顺序 = link 顺序 | 链输入流 = 确认序 × DFS 序；只消费已确认输入 |
...
| 孤块池 | `MAX_ORPHAN_SIZE = 3750`，满则拒绝账户交易块 | 需改为按链分队列/提高上限（SP0） |
| 原生账本 | `AddressStore`：20B 地址 → 余额/nonce | 链金库、保证金库都是普通 20B 地址（无私钥）；用户在链内沿用同一地址 |
| P2P | `MessageCode` 最大 `0x1A`；新块经 `NEW_BLOCK` gossip 与 sum 树同步 | 新消息码自 `0x1B` 起；不新增块传播协议 |
| 存储 | 8 个 RocksDB 实例，1 字节前缀分区，只有 `batchWrite` 原子 | 新增 `CHAIN_L1`（全网小状态）与 `CHAIN_EXEC`（订阅链状态）两个实例 |
...
| 层 / 角色 | 职责 | 谁运行 |
|-----------|------|--------|
| **L1 DAG** | 排序、DA、原生账本、链注册表、保证金账本、金库、锚定规范链、欺诈证明仲裁 | 全部节点 |
| **链 Chain（应用链）** | 状态分区 + 执行单元，1..N 个 WASM 合约；链内同步原子调用；状态 = L1 输入流的纯函数 | 订阅该链的节点 |
| **执行者** | 订阅若干链，计算其状态，提供 RPC/证明服务 | 任意节点 |
| **认证者 Attester** | 锁定保证金，发布锚定块 | 任意执行者 |
| **挑战者** | 发现错误锚定，提交挑战块 | 任意执行者 |
| **认领者** | 为终局提现/跨链转值提交 CLAIM（可由机器人代劳） | 任意人 |
...
```
 ...
 L1 DAG ── NEW_BLOCK gossip ── 主块 link ── setMain 确认 ── applyBlock DFS
   │  值结算：INPUT 用户 → OUTPUT 链金库（现有路径）
   │  L1 钩子：记录链输入 / 注册表 / 保证金 / 锚定规范链 / 挑战仲裁 / 认领划账
   ▼
 链执行者（只订阅链 L）
 ...
 认证者：ANCHOR 块（stateRoot, outboxMapRoot, 承诺链）──► L1 规范锚定链 ──W──► 终局
   ...
   └── 挑战者：CHALLENGE 块（callIndex + 见证）──► L1 单步重执行仲裁 ──► 罚没/作废
                                                       │
 认领者：CLAIM 块（终局锚定 + MMR 证明）──► L1 金库 → 收款人 / 目标链金库
```
...
5. **状态分区沿 DAG 边界**：链之间只通过消息（outbox 条目）耦合，没有共享世界状态。
...
- 激活高度 `chain.activationHeight` 之前，所有节点忽略扩展语义（与旧节点行为一致）。
- **EXT 不影响 L1 有效性（SP0a 决策 2026-09-13）**：`tryToConnect` 不读扩展字段，格式错误的扩展块仍是合法 L1 块并正常结算值；链语义只在 `setMain`/`applyBlock` 按主块高度激活后解释，格式错误的调用记为 `INVALID_FORMAT` 输入（value 退回链内余额）。导入期的分片费率与配额检查是节点本地反垃圾策略（SP0b），不是共识。
...
|------|------|------|------|
| 0 | 1 | kind | 0x02 |
| 1 | 1 | flags | bit0 = 新建链；bit1 = 代码走分片链（否则 codeHash 必须已在代码库）；bit2 = init 参数走分片链 |
| 2 | 20 | chainId | 加入已有链时的链 id；新建时忽略 |
| 22 | 4 | gasLimit | init 调用 gas 上限 |
| 26 | 2 | argsLen | 内联 init 参数长度 |
| 28 | 4 | 保留 | 0 |

载荷字段：payload[0] = codeHash（32B，原始 WASM 字节的 sha256）；payload[1] = 链配置（仅新建链时有效）：`gasPrice u64（nano/gas） ‖ deliveryDelayD u32 ‖ maxCallGas u32 ‖ 保留 16B`；其后为内联 init 参数。link[0] = 代码分片链首片（flags.bit1）；link[1] = init 参数分片链首片（flags.bit2）。

派生：新链 `chainId = sha256("xdag-chain" ‖ blockHash)[0..20]`；新合约地址 `contract = sha256("xdag-contract" ‖ blockHash)[0..20]`。新链的金库地址就是 chainId，在 `setMain` 时以 0 余额创建于 `AddressStore`。
...
|------|------|------|------|
| 0 | 1 | kind | 0x04 |
| 1 | 1 | flags | 保留（bit0 预留给 v2 编织多链锚定） |
| 2 | 20 | chainId | |
...
link[0] = 上一规范锚定（创世锚定为链的 DEPLOY 块）；link[1] = 高度 h 的主块；link[2] = 承诺链首片（§9.2，内容 = 已消费段列表 ++ 逐输入承诺 C_1..C_n）。
...
| 2 | 20 | chainId | 全零 = 全局保证金（可为任意链锚定） |
| 22 | 8 | amount | UNBOND 时的解锁额；BOND 时忽略（以 OUTPUT 金额为准） |
| 30 | 2 | 保留 | 0 |

BOND：INPUT 认证者 → OUTPUT 保证金库地址 `CHAIN_BOND_VAULT = sha256("xdag-chain-bond")[0..20]`，amount = 保证金。UNBOND：自转 MIN_GAS 付费；到期（确认高度 + W）在 `setMain` 时由系统从保证金库划回 `min(申请额, 剩余保证金)`（期间被罚没则相应减少），无需额外块。
...
|------|------|------|------|
| 0 | 1 | kind | 0x07 |
| 1 | 1 | flags | bit0 = 收款方是链金库（跨链转值对账）；否则收款方是 L1 用户地址 |
| 2 | 20 | srcChain | 源链 |
| 22 | 8 | seq | 所引用终局锚定的 seq |
| 30 | 2 | 保留 | 0 |

payload[0] = `index u64 ‖ amount u64 ‖ 保留 16B`；payload[1] = `recipient 20B ‖ 保留 12B`。link[0] = 终局锚定；link[1] = 证明分片链首片（对 outboxMapRoot 的 SMT 证明 + 对目标 MMR 的成员证明，§7.6）。值结算：自转 MIN_GAS 付费。字段：header + nonce + INPUT + OUTPUT + pubkey + sig×2 + 扩展头 + 2 载荷 + 2 link = 12。系统动作：金库(srcChain) → recipient 划账 `amount`。

### 5.3 分片链的引用与 DA

- 付费块 link 首片，首片 link 次片，……链尾无 link。`tryToConnect` 的 `NO_PARENT` 规则保证：付费块可导入 ⇔ 整条链已在本地。
- 分片块本身是 EXTRA/孤块，未被任何付费块引用时可被孤块池按现有规则淘汰；被主块直接 link 的"裸分片"只是无意义的 link block，不进入任何链语义。
- 单条链上限 `chain.maxChunksPerChain`（初始 4096 片 × 352 B ≈ 1,441,792 B ≈ 1.44 MB）。
...
扩展块的 header fee 字段必须满足：`header.fee ≥ chunkFee × 该块直接 link 的所有分片链总片数`。`chunkFee` 为协议参数（初始 0.01 XDAG/片）。两层执行：导入期作为节点本地策略拒绝（SP0b，可配置）；`applyBlock` 时作为共识复核，不足者记为 `INVALID_FEE` 输入（value 退回链内余额，分片链不被使用）。这笔费用与现有交易费一样归 PoW 主块矿工，覆盖排序与 DA 成本。复核时 `header.fee` 必须从**原始 512 字节的 header 字段**读：`BlockInfo.fee` 在 apply 期间会被改写成"这个块收到的手续费"，不再是"这个块声明了多少"。
...
**参数链的费用基数是宽松计数（SP0a 决策 2026-09-14）**：L1 只装配 DEPLOY 的代码链，它的片数必须在装配通过之后再数；CALL 的参数链与 DEPLOY 的 init 参数链 L1 **从不装配**（SP1 的执行引擎才装配，装不出来就在链内失败并退款），它们的费用基数就是"查得到 + 够新 + 能解码成 CHUNK 的片数"这个宽松计数。分片费收的是全网确实要存的分片块，与这条链将来能不能装配无关。

### 5.5 兼容性与硬分叉点

- **软兼容部分**：旧节点解析 `0x0F` 走 `default`，把 CALL/DEPLOY/ANCHOR/BOND/CHALLENGE/CLAIM 当普通账户交易结算，哈希/签名不受影响。
- **硬分叉部分**（激活高度后行为分歧）：(a) CLAIM 与 UNBOND 到期的**系统划账**改变 L1 余额；(b) 快照必须携带 `CHAIN_L1`；(c) 孤块池、导入流水线与导入期费率策略均为节点本地，不影响共识。因此 `chain.activationHeight` 必须作为硬分叉高度统一升级，与 dev-evm 的 `*ActivationHeight` 治理方式相同。
- **激活前的资金陷阱**：旧节点把 CALL 当普通转账，value 进金库却没有链语义，激活前发出的调用资金无法 CLAIM。因此钱包、SDK 与 `chain_*` RPC 在激活高度前必须拒绝构造/提交扩展块；协议侧只从激活高度起记录链输入，激活前进入金库的余额视为捐赠，不做补救。

---

## 6. L1 状态扩展与钩子

### 6.1 全网小状态（`CHAIN_L1` RocksDB 实例）

| 前缀 | key | value | 用途 |
|------|-----|-------|------|
| 0x01 | chainId | 配置（gasPrice, D, maxCallGas）、创建高度、创建块 hash | 链注册表 |
| 0x02 | contract(20) | chainId ‖ codeHash ‖ 部署高度 | 合约 → 链 |
| 0x03 | codeHash | 原始 WASM 字节（只有 blob） | 代码库（全网保存，仲裁需要） |
| 0x04 | attester(20) ‖ chainId | 保证金额 ‖ 解锁申请高度 | 保证金账本 |
| 0x05 | chainId | 规范头 seq ‖ 锚定块 hash | 锚定链头 |
| 0x06 | chainId ‖ seq | 锚定记录：hash、stateRoot、outboxMapRoot、inputCount、确认高度、状态（canonical/final/voided） | 锚定索引 |
| 0x07 | chainId ‖ height | `callCount u32` | 每高度输入计数（DFS 时顺手统计） |
| 0x08 | srcChain ‖ dstKey ‖ index | 1 | 已认领防重放 |
| 0x09 | height | 待到期列表（UNBOND、锚定终局） | 高度触发器 |
| 0x0A | challengeHash | 仲裁结果 | 挑战记录 |
| 0x0B | dstChain ‖ srcChain | 已消费游标 `to`、已消费到的 srcAnchorSeq | 段消费游标（§7.5） |
| 0x0C | chainId ‖ height ‖ index | blockHash ‖ kind ‖ status ‖ contract | 输入索引（执行者按此重建输入流） |
...
| 0x0E | blockHash | chainId ‖ height ‖ index 的定长数组 | unapply 反向索引（一个块可以有多条输入记录） |

`index` 是**每 (chain, height) 内从 0 递增的序号**（0x07 的 `callCount` 就是下一个 index），不是主块级的全局 DFS 序号：执行者按 `0..callCount−1` 枚举该链该高度的输入流。
...
| `onBlockApplied`（`applyBlock` 置 `BI_APPLIED` 后，DFS 序） | CALL/DEPLOY：登记链输入（0x0C/0x0E/0x07；DEPLOY 更新注册表/合约表/代码库 0x01/0x02/0x03/0x0D）；BOND/UNBOND：更新保证金账本；ANCHOR：有效性检查 → 规范链；CHALLENGE：仲裁（§10）；CLAIM：证明校验 → 系统划账 | `onBlockUnapplied`（`unApplyBlock` 逆序）逐项反向 |
...
XDAG 主网节点通常从快照启动。`CHAIN_L1` 的全部内容（注册表、合约→链、代码库、保证金账本、锚定链头与索引、每高度输入计数、已认领集合、高度触发器、段游标）必须随快照一起导出/导入，并在快照中附带一个 `chainStateHash = sha256(CHAIN_L1 全部 KV 的规范序列化)` 供校验；快照启动的节点若在激活高度之后缺少该段则拒绝启动。理由：dev-evm 曾因 alloc 标记未进快照导致快照节点与全量节点状态分歧，本设计的 L1 小状态直接影响锚定有效性判定与系统划账，缺失即分叉。

SP0a 的落地形态（细节见 SP0a 规格 §8）：快照从此有**三个目录** `SNAPSHOT/BLOCKS`、`SNAPSHOT/ADDRESS`、`SNAPSHOT/CHAIN_L1`，导出无条件进行（空的 `CHAIN_L1` 导出的就是"元数据 + 哈希"）；导入闸门是快照启动分支的第一条语句，与 SnapshotJ 开关无关，fail-fast；一次成功的导入会在本地库里留下一个**一次性持久标记**（`0xFF`，不计入状态哈希）；这道门只在块存储被快照重新灌库时到达（普通重启走不到），标记存在时还必须同时证明"本次用的是同一份快照"（快照记录的哈希 = 标记）与"本地未漂移"（`stateHash()` = 标记）才幂等返回，否则拒绝并提示删本地 `CHAIN_L1` 目录；本地有状态却没有标记同样拒绝启动。**快照里不携带任何原始分片字节**：年龄规则允许付费块往回够两个 epoch，可能够到快照之前，那些分片块由 `NO_PARENT` 触发的按需拉取补齐（见 §20.2 E12）。
...
systemInputs(L, h):      高度 h 成为规范的 L 的锚定各产生一条"锚定奖励"输入 {attester, prevSeq, seq}，
                         执行效果 = 把 (prevSeq, seq] 的费用池划给 attester 的链内余额；inputHash = sha256("reward" ‖ anchorHash)
...
链状态 `S(L, h) = exec(S(L, h−1), inputs(L, h))`。`seq` 即 h。三类输入的**数量与身份 L1 都能确定**（规范锚定、可交付段、已结算块都是 L1 状态），只有段内消息的内容需要源链数据。由于 inputs 只依赖 L1 已确认历史与其他链的**已确认锚定**，`S(L, h)` 是 L1 历史的纯函数：任何两个诚实执行者必然得到相同结果。

### 7.2 单笔 CALL 的事务语义

1. 前置：value 已由 L1 结算进金库；链内账本 `bal[sender] += value`。
...
4. 结束：成功 → 提交状态；失败（trap / abort / 超 gas / 超深度）→ 回滚到调用前，`status = FAILED`。两种情况都退还未用 gas：`bal[sender] += (gasLimit − gasUsed) × gasPrice`；已用 gas 计入链费用池。
...
- `send(dstChain, contract, selector, args, value)`：向本链 outbox 追加条目 `{srcChain, dstChain, index, sender, contract, selector, args, value, srcHeight}`（`sender` = 发起 send 的合约地址或用户地址）；`bal[sender] −= value` 进入"出站在途"。
- 目标链收到消息作为输入时：先 `bal[sender] += value`（在目标链内以原发地址记账），再执行 `recv`；`recv` 内用 `transfer_from_caller` 领取。`recv` 失败则状态回滚，value 留在目标链内的 `bal[sender]`，原发者可提现回 L1 或再次 `send`。
- 消息**不做回执**（无 reply）；需要回执的应用自行反向 `send`。

### 7.5 交付规则与 D

- 源链 S 的每个规范锚定 A（seq = h_S）在其 outboxMap 中给出每个目标链 X 的 MMR 大小 `size_A(X)`；段 `[size_{prev}(X), size_A(X))` 是 A 新增的发往 X 的消息。L1 只看得到 outboxMapRoot，看不到 `size_A(X)`，因此段的上界 `to` 由目标链的认证者**声明**，其真实性由 E3 挑战保证（§10.1）；下界 `from` 必须等于 L1 记录的该 (X, S) 游标（上一次消费的 `to`），可被 L1 精确核对。
- 段对 X **可交付**的高度 `h_deliver = confirmHeight(A) + D_X`，其中 `confirmHeight(A)` 是 A 所在主块的确认高度，`D_X` 是 X 的配置。
- **消费是强制且确定的**：X 在高度 h 必须消费所有 `h_deliver ≤ h` 且尚未消费的段，按 (A 的 L1 确认序) 排序。锚定中"已消费段列表"（§9.2）的 (srcChain, srcAnchorSeq) 序列与各段 `from` 必须与 L1 计算结果完全一致，否则锚定结构无效；`to` 可挑战。
- 若 A 后来被欺诈证明否决，所有消费了 A 的段的链从消费点回滚（级联）。`D_X = W` 时永不级联；`D_X = 0` 时即时交付但暴露于级联，适合同一运营方的链群。
...
| 链内账本 | 视为系统合约 `0x00…00` 的 KV：`bal[addr]`、费用池 |
| stateRoot | 二叉**稀疏 Merkle 树**（256 层，空子树哈希预计算），哈希 = SHA-256（xdagj 已有；blake3 作为备选，若基准显示瓶颈可在激活前更换，激活后不可变） |
| outbox | 每个目标（dstChain，或 `L1 = 全零`）一棵 append-only **MMR**；叶 = `sha256(条目编码)` |
| outboxMapRoot | 一棵小 SMT：key = dstChain，value = `mmrRoot ‖ size u64` |
...
| `caller(ptr)` / `self_address(ptr)` / `chain_id(ptr)` | `(i32)->()` | 20B 写入 | 10 |
...
| `balance(aptr)` | `(i32)->i64` | 链内余额 | 100 |
| `transfer(aptr,amount)` | `(i32,i64)->i32` | 链内转账 | 500 |
| `transfer_from_caller(amount)` | `(i64)->i32` | 领取本次 value | 200 |
| `withdraw(aptr,amount)` | `(i32,i64)->i32` | 向 L1 地址提现（outbox L1 条目） | 2000 |
| `call(cptr,sel,aptr,alen,value,rptr,rcap)` | `(i32,i32,i32,i32,i64,i32,i32)->i32` | 同链同步调用 | 500 + 被调 gas |
| `send(lptr,cptr,sel,aptr,alen,value)` | `(i32,i32,i32,i32,i32,i64)->i32` | 跨链消息 | 2000 + 50/32B |
...
- **执行 gas** = `gasUsed × gasPrice(chain)`，从 sender 链余额扣，进入链费用池；在锚定 seq = h 成为规范时，`(prevSeq, h]` 内的费用池归该锚定的认证者（链内余额，可提现）。
- 链 `gasPrice` 由部署者在创建时设定，协议下限 `minGasPrice`（初始 1 nano/gas）。
...
- BOND 后即刻生效；`bond(attester, chain) + bond(attester, 全局) ≥ minBond`（初始 10,000 XDAG，待经济分析）才可为该链锚定。
...
1. **已消费段列表**：`segmentCount` 条，每条 `srcChain 20B ‖ srcAnchorSeq u64 ‖ from u64 ‖ to u64`（44B，紧凑拼接后按 32B 字段切分）。
...
| 检查 | 依据 |
|------|------|
| 签名者保证金足额 | `CHAIN_L1` 0x04 |
| link[0] 是当前规范头且 `prevSeq` 匹配，`seq > prevSeq` | 0x05 |
| link[1] 是高度 `seq` 的规范主块 | `BLOCK_HEIGHT` 索引 |
| 已消费段列表的 (srcChain, srcAnchorSeq) 序列与各段 `from` == L1 按 §7.5 计算的结果 | 0x06 + 0x0B 游标 + 各源链配置 |
...
通过即成为 `(chain, seq)` 的**规范锚定**（同 seq 后来者忽略）。

### 9.4 终局

锚定所在主块确认高度 + W（初始 32）到达且状态仍为 canonical → `final`。CLAIM 只认 final 锚定；`D_X = W` 的链只消费 final 段。

### 9.5 否决与回退

挑战成功 → 该锚定及其所有后代置 `voided`，规范头回退到其父；消费了其段的链级联回滚（§7.5）；新的锚定可以从父继续。认证者的保证金按 §10.4 罚没。
...
WitnessV1 {
  ...
  input {                          // E1 / E2
    kind u8, blockHash 32B | message { srcChain, index, 条目编码, MMR 证明, outboxMap SMT 证明 }
  }
...
verify(challenge):
  ...
  // 2. 输入正确性 (E1)
  expected = expectedInputAt(A.chain, A.prevSeq, A.seq, k)  // L1 可算：段列表 + callCount；消息身份用见证证明
...
| 跨链转值 | `send(value)` → outbox(dst) 条目 → 目标链消费即入账 → 任意人 CLAIM(flags.bit0) | 金库(src) → 金库(dst) |
| gas | 链内扣，归认证者 | 无 |

**不变量 I1**（每链）：`Σ 链内余额 + 出站在途 − 入站在途 == 金库 L1 余额`，其中"在途"以已终局锚定为界。
...
| 垃圾挑战 | 押金没收；每高度仲裁数上限 |
| 冷门链无人观察 | 用户可自行运行执行者；`maxWithdrawPerWindow ≤ f(总保证金)`（可选参数）限制单窗口最大损失 |
| 分片链垃圾块 | 分片块无费用但只有被付费块 link 才有语义；付费块按片数付费；孤块池按现有规则淘汰 |
| 级联回滚攻击（源链故意锚错让下游回滚） | 下游自选 D；D = W 免疫；源认证者被罚没 |
| L1 深 reorg | 链按变更日志回滚；锚定 link 主块自然失效 |
| Chicory 解释器与编译器分歧 | 仲裁只用解释器；执行者分歧只产生可挑战锚定 |
| WASM 非确定性 | §8.1 白名单 + 插桩；部署期拒绝 |
| 资源耗尽（内存/栈/gas） | 硬上限 + trap；仲裁有界 |
| 部署者把链 `maxCallGas` 设得过大（或为 0），使仲裁无界 / 链永远跑不了调用 | L1 在 DEPLOY（新建链）处强制 `1 ≤ maxCallGas ≤ 10,000,000`（= §17 的 `maxCallGas` 初值，只能下调；实现为 `ChainL1Processor.MAX_CALL_GAS_CAP`），越界记 `INVALID_FORMAT`，链不注册。该检查先于代码链装配。`gasPriceNano` 与 `D` 由部署者自定，不设上下界 |

**信任假设汇总**：PoW 主链诚实多数（不变）；每条链在任意 W 窗口内至少一个诚实观察者在线；出主块的矿池是事实上的排序者，其抢跑与短期审查能力与今天 XDAG 交易的情况相同，本设计不消除它（§20.1 S3）。
...
2. 执行者收到 `onUnwind(h0)` → 每个订阅链回滚到 seq ≤ h0。
3. 重新 `setMain` 后按新输入流重放。
4. **属性测试**：对任意块序列与任意 unwind 点，`apply → unwind → apply` 的 L1 状态与链状态等于直接 `apply`。
...
| 消息码 | 名称 | 方向 | 内容 |
|--------|------|------|------|
| 0x1B | CHAIN_SUBSCRIPTIONS | 握手后互发 | 本节点订阅的 chainId 列表（路由提示） |
| 0x1C / 0x1D | CHAIN_SNAPSHOT_REQUEST / REPLY | 请求 (chain, seq, page) | 快照分页（KV 批 + 页哈希）；请求方核对根 == 该 seq 规范锚定 stateRoot |
| 0x1E / 0x1F | CHAIN_PROOF_REQUEST / REPLY | 请求 (chain, seq, kind, key) | outbox 段内容 + MMR 证明、outboxMap SMT 证明、C_k 开解、事件 |

- 链块传播零新增：CALL/CHUNK/ANCHOR/… 走 `NEW_BLOCK` gossip 与 sum 树同步。
- 新订阅者引导：找最近 final 锚定 → 拉快照 → 校验 → 从 seq 重放本地 L1 输入流；无锚定链从 DEPLOY 块重放。
- 执行者只对订阅链保存 `CHAIN_EXEC` 状态；未订阅链仅保存 `CHAIN_L1` 中的锚定索引。
...
| 层 | 上限 | 备注 |
|----|------|------|
| 执行 | 无 | 链独立；每个执行者的吞吐 ≈ 核数 × 单核 WASM 吞吐；加链加执行者即扩容 |
| 状态 | 无 | 仅存订阅链 |
| 排序 + DA | 单节点导入带宽 | 每笔调用 = 512B 块 + 32B 承诺 ≈ 544B；10k 块/s ≈ 5.4MB/s |

### 15.2 L1 导入流水线（SP0）

- **锁外预验证池**：解析、哈希、EXT 校验、分片链结构、签名验证（`verifiedKeys` 缓存）在线程池完成；按 `chainId`（或 sender）分片的工作队列保证同链顺序。
- **锁内最小化**：`tryToConnect` 只做存在性、link 校验、难度计算、DAG 插入、孤块池更新。
- **批量落盘**：RocksDB `WriteBatch`。
- **孤块池**：从单一 `MAX_ORPHAN_SIZE = 3750` 改为按链/发送方的分队列与全局上限（初始 100k）。
...
| 方法 | 说明 |
|------|------|
| `chain_sendCall(rawBlocks[])` | 提交客户端签好的 CALL 块与分片链 |
| `chain_query(chain, contract, selector, args, view)` | 只读执行 `query`，`view ∈ {pending, confirmed, anchored, final}` |
| `chain_getReceipt(blockHash)` | 状态、gasUsed、返回数据、事件、所在 seq、四级确认状态 |
| `chain_getAnchor(chain, seq)` / `chain_getHead(chain)` | 锚定索引 |
| `chain_getState(chain, contract, key, view)` / `chain_getProof(...)` | 状态与 SMT 证明 |
| `chain_getBalance(chain, address, view)` | 链内余额 |
| `chain_estimateGas(...)` | 在 `pending` 视图试执行 |
| `chain_subscribe(receipts | events | anchors)` | WebSocket 推送 |
| `chain_buildClaim(srcChain, seq, index)` | 组装 CLAIM 所需证明 |
...
- CLI：`xdag chain create|deploy|call|query|bond|unbond|anchor|challenge|claim|subscribe`。
- 参考 DApp：XRC-20 代币；单链 DEX（池 + 代币同链 → 原子撮合）；跨链消息演示（D = 0 与 D = W 对照）。
...
| 参数 | 初始值 | 说明 |
|------|--------|------|
| `chain.activationHeight` | devnet 0 / testnet, mainnet MAX | 硬分叉高度 |
| `W` | 32 主块（≈ 34 min） | 挑战窗口 = 终局延迟 = UNBOND 延迟 |
| `minBond` | 10,000 XDAG（待校准） | |
| `minChallengeDeposit` | minBond / 10 | |
| `slashSplit` | 50% 挑战者 / 50% 销毁 | |
| `D`（链默认） | W | 部署时可设 0..W |
| `maxCallGas` | 10,000,000（待校准，只能下调） | 单次调用与单次仲裁上限。同时是**每链配置的硬上界**：L1 在新建链的 DEPLOY 处强制 `1 ≤ maxCallGas ≤ 10,000,000`，越界 → `INVALID_FORMAT`（§12） |
...
| `maxChunksPerChain` | 4096 | 4096 × 352 B ≈ 1.44 MB。与 `maxWasmBytes`、`chunkFee`、`activationHeight` 一样是**共识参数**：SP0a 给了 conf 键（`chain.chunk.maxPerChain` / `chain.wasm.maxBytes` / `chain.chunk.feeMilliXdag` / `chain.activation.height`）只为 devnet 与测试，**共享网络上绝不设置**——非默认值不会报错，只会静默分叉 `CHAIN_L1`；被覆盖时启动打 `warn` |
...
| `maxWithdrawPerWindow` | 开启，公式由 SP3 定（初始建议 ≤ 该链总保证金 × 2 / W 窗口） | 纵深防御（§20.1 S2） |
...
| # | 子项目 | 核心交付 | 依赖 |
|---|--------|---------|------|
| **SP0a** | 块格式与 L1 钩子（共识层） | `XDAG_FIELD_EXT` 编解码；7 种 kind 解析与校验；分片链；`CHAIN_L1` 存储 + `batchWrite`；注册表/合约表/代码库（引用计数）/输入索引；五个 setMain/applyBlock 钩子与对称 unapply；激活高度与 `ChainSpec`；`ChainActivation` 供钱包/RPC 门控；**快照扩展与 chainStateHash（§6.3）**；属性测试。详见 `2026-09-13-xdag-chain-sp0a-block-format-and-l1-hooks-design.md` | — |
| **SP0b** | 导入流水线与孤块池（节点本地） | `tryToConnect` 锁外并行预验证（签名、解析）；孤块池按链/来源分队列、分片块配额与 TTL 淘汰、`orphanPoolLimit`；导入期分片费率策略；L1 导入基准（块/s 前后对比） | SP0a（可与 SP1 并行） |
| **SP1** | 链执行引擎 | Chicory 集成；部署验证 + 插桩（含栈高度计量与保守 `-Xss` 无关上限）；宿主 ABI；SMT/MMR；**见证比例计费与 §8.3 不等式的属性测试**；输入流构建；确定性执行；变更日志/回滚/快照；软预执行；两模式一致性测试；gas 微基准 | SP0a |
| **SP2** | 资产流 | 金库记账；withdraw/outbox(L1)；CLAIM 校验与系统划账；跨链转值对账；不变量 I1–I3 测试 | SP1 |
| **SP3** | 认证与仲裁 | BOND/UNBOND；锚定有效性与规范链；承诺链；终局 W；见证格式；E1/E2/E3 仲裁；**锁外预计算裁决缓存**；罚没；级联回滚 | SP1（与 SP2 并行） |
| **SP4** | 订阅与 P2P | 订阅集合（含 `chain.subscribeAll` 过渡模式，D6）；`0x1B–0x1F` 消息；快照同步；证明拉取；交付规则 D；执行者只跑订阅链；多节点 devnet | SP2, SP3 |
| **SP5** | 开发者面 | `chain_*` RPC；Rust SDK；客户端库；CLI；三个参考 DApp；文档 | SP4 |
| SP6（v2） | 扩展 | 编织原子跨链锚定（ANCHOR flags.bit0）；BATCH 密度块；DA 采样 | v1 上线后 |

顺序：SP0a → SP1 → SP2 / SP3（SP1 后可并行）→ SP4 → SP5；SP0b 与 SP1 并行。

**实施级总体设计与路线图**（各子项目的接口、数据结构、算法、测试、切片与里程碑）见 `2026-09-17-xdag-chain-contracts-program-design-and-roadmap.md`；SP0b 拆为 SP0b-1（基准 + 加固工单）→ SP0b-2（锁外预验证流水线）→ SP0b-3（孤块池分队列/配额/TTL + 导入期费率策略）。
...
| Reorg | 随机深度 unwind 后链状态与金库守恒；锚定 link 主块失效 |
| 基准 | L1 导入块/s（并行化前后）；多链执行核数扩展曲线；SMT 更新吞吐；单次仲裁耗时上限 |
| 端到端 | 3 节点 devnet、2 条链、节点订阅不同链；跨链 DEX 演示；一次人为错误锚定的完整挑战-罚没流程 |
...
| # | 风险 | 影响 | 缓解 / 现状 |
|---|------|------|------------|
| S1 | **延迟**：主块 64s、确认约 2 epoch，调用要 2–3 分钟才进入链输入流；跨链再加 D，提现再加 W（≈ 34 min） | DeFi 体验明显弱于秒级链 | 软预执行只改善展示；终局时间不可缩短，除非改主链节奏（非目标）。诚实地把它写进产品预期 |
| S2 | **冷门链无人看守**：安全前提是每条链在 W 内有诚实观察者；金库 ≫ 保证金时理性认证者有作恶动机；终局锚定不可挑战，一旦得逞永久损失 | 长尾链资金风险 | `maxWithdrawPerWindow ≤ f(总保证金)` 默认**开启**（改自"可选"，SP3 定公式）；钱包对无独立观察者的链给出风险提示；社区运行公共观察者 |
| S3 | **矿池即排序者**：DFS 顺序由出主块的矿池决定，矿池天然拥有 DEX 抢跑权，也能拖延 CHALLENGE 块确认；XDAG 矿池集中度高，矿池很可能同时成为主要认证者 | MEV 与中心化压力 | 链可选"epoch 内按块 hash 排序"的反 MEV 模式（v2 研究项）；W 取 32 而非更小，使拖延挑战需要长时间多数算力；把风险写入 §12 信任假设 |
| S4 | **可组合性碎片化与引力效应**：没有原子跨链，代币与 DEX 会挤进同一条大链，实际吞吐退化为单执行者上限，"无上限"被引力抵消 | 实践中的 TPS 远低于理论 | v2 编织原子跨链锚定（ANCHOR flags.bit0 已预留）；SDK 提供跨链代币/流动性模式；单链执行引擎本身做多核并行（同链内互不冲突的合约可并行，v2） |
| S5 | **级联回滚**：D=0 链群在上游锚定被否决时连锁回滚，开发者要理解"已确认但可撤销"的状态语义 | DApp 难写对（Vite 教训） | 链内同步作为主编程模型；SDK 只暴露 `confirmed/anchored/final` 三级明确语义；D=0 需部署者显式声明 |
| S6 | **经济参数无市场**：gasPrice 由部署者定死、L1 块费固定、保证金与罚没比例靠估计，激活后只能硬分叉调整 | 定价失真、参数僵化 | v2 引入链级 gasPrice 治理（部署者可更新）与 L1 费率市场；激活前用 testnet 数据校准 |
...
| E3 | 快照启动节点缺 `CHAIN_L1` 导致分叉 | §6.3 快照扩展 + `chainStateHash`，纳入 SP0 |
...
| E9 | 保证金经济：`minBond` 与链可提现额的关系 | SP3 经济分析；S2 的提现限速默认开启 |
...
| E13 | **P3（写必有成对反写）存在两处已知缺口**，SP1 不能无条件假设它成立 | (a) `BlockchainImpl.unApplyBlock` 会跳过 `BI_MAIN_REF` 已置而 `ref == null` 的块（`setMain` 在 DFS 里抛异常、或走 `mainBlockFee < 0` 的提前 `return` 时会留下这种块），其子块已提交的 `CHAIN_L1` 记录永不撤销——这是既有的值结算不对称（同样这些块的余额也保留着），不是链层引入的，但 SP0a **放大了它的触发面**：`CHAIN_L1` 是第二个 RocksDB，`applyBlock` 的 DFS 里会 `commit` 它，且记录损坏是故意的 fail-stop，DFS 里因此多了一类抛出点；`checkMain` 吞掉异常，留下的主块 `BI_MAIN`/高度/奖励/`nmain++` 都已做完而 `updateBlockRef` 没走到，永远 unwind 不了。SP0b 负责启动一致性检查与让这个跳过可修复；(b) `BI_APPLIED` 在 `CHAIN_L1` 提交之前就已落盘，两个库之间没有原子性也没有启动重放，崩在中间会丢掉那个块的链记录。(b) 与 `ADDRESS`/`BLOCK` 两库本来就不原子是同一档次，**接受**；SP0b 或快照工具可加一道启动时的 `CHAIN_L1` vs `BLOCK` 一致性检查 |

### 20.3 生态与交付风险

| # | 风险 | 处理 |
|---|------|------|
| D1 | **冷启动**：无 Hardhat/MetaMask/Etherscan，只有 Rust SDK；NEAR、Polkadot 最终都补回了 EVM 层 | 链抽象与 VM 无关：v2 可以引入"EVM 链"（把 dev-evm 的执行器装进一条链），不违背 L1 设计；v1 先做 Rust SDK + AssemblyScript |
| D2 | **交付风险**："一步到位分片"让 v1 覆盖 SP0–SP5，任一子项目卡住整个方向无法上线 | **已决定（2026-09-13）纳入 v1**：SP4 提供 `chain.subscribeAll` 配置，节点订阅全部链时行为退化为"全复制并行执行"，可作为过渡模式先上 testnet；分片订阅、认证与仲裁在同一代码路径上逐步启用 |
...
| 术语 | 含义 |
|------|------|
| 链 Chain（应用链） | 状态分区 + 执行单元；有唯一 20B chainId，同时是其金库的 L1 地址 |
| 输入流 | 链在某高度必须消费的有序输入（消息 ++ CALL/DEPLOY） |
| seq | 链步号 = 主块高度 |
| 承诺 C_k | 第 k 个输入执行后的 32B 承诺 |
| 锚定 Anchor | 认证者对某 seq 状态的签名声明 |
| 规范锚定 | 每 (chain, seq) 中 L1 确认序第一个有效锚定 |
| 终局 final | 锚定确认后经过 W 且未被否决 |
| 段 | 源锚定新增的发往某目标链的消息区间 `[from, to)` |
| D | 目标链对入站段的交付延迟 |
| W | 挑战窗口 |
| 见证 | 单步重执行所需的全部证明 |
| 金库 | 链的 L1 地址（无私钥）及其余额 |
