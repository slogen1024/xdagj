...
- **交付后修订（2026-08-18，审查裁定的两处加固）**：①矿工打包侧（spec 原文未覆盖的接缝）
  增加激活前 type-2 过滤——若被打包执行成 status-0，同字段重签哈希不变，收据去重会令该
  哈希**永久不可执行**且 sender nonce 链停滞，故 createMainBlock 的两个选择器按预测高度
  跳过（执行侧兜底保留）；②预算记账镜像——budget peek 对激活前 type-2 返回 -1（未升级
  节点视角），否则预算吃紧的块中后续 ref 的执行/跳过会在新旧节点间分歧，破坏本节
  逐字节一致性质。
