...
| G5 | `BlockchainImpl` 在构造器里起了非 daemon 的 `rollBackLoop` / cleaner 线程，`stopCheckMain()` 从不停它 | 既有问题；测试基座每个测试泄漏一个线程。**已关闭（SP0b-1 T5）**：`stopCheckMain()` 现经 `stopCleaner()` 停掉 cleaner；`--repairchain` 依赖它退出（提交 3877c1b5） |
