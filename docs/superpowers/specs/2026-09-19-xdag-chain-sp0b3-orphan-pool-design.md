...
**但上表说的是「选取名额」的顺序，不是「发射字段」的顺序。两者必须分开，否则会破坏真实交易。**（Task 9 实测补充。）

`applyBlock` 按字段顺序遍历一个块，而一笔比发送方已执行 nonce 超前一个以上的交易会被**永久拒绝**——那条检查会调 `resetTxQuantity`，并留下 `BI_MAIN_REF`，该块再也不会被重新考虑。而对端的 link 块经常携带着本节点池中那笔 nonce 为 n+1 交易的前驱（nonce 为 n）：导入那个 link 块，正是把前驱从池里移走的原因。

所以若把账户交易排在 link 块**之前**发射，前驱还没被应用，后继就被永久判死。实测：`testLinkAndNonceImpactOnSorting` 中五个确认变成永久拒绝，标志位 `0x1c` → `0x18`（已用插桩排除资金不足，余额均为 1000 XDAG）。

正确做法是：账户交易与 mtx **先拿到名额**（选进暂存列表），但**追加在 link 填充之后发射**。主块最终收到的字段顺序是 `mainRef → VIP → links → 账户交易 + mtx`。饿死要解决的是交易拿不到名额，不是它在字段里排第几。
