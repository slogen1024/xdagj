...
### 5.2 SP0b-2：锁外预验证流水线与批量落盘（**已实施，见 SP0b-2 规格与计划**——`2026-09-18-xdag-chain-sp0b2-ingest-pipeline-design.md` v2 是竣工版，本节保留原方案，与实现不一致处以该规格为准；**验收目标 ≥ 10,000 块/s 未达成，实测中位 7157 块/s**，原因与下一步见该规格 §4.3 与 `docs/benchmarks/2026-09-19-l1-import-pipeline.md`）
...
### 5.2.1 SP0b-2b：锁内"其余"的压缩（**新增，未开始**；SP0b-2 验收未达成的直接后续）

SP0b-2 把验签与落盘都搬出了锁，导入吞吐从 SP0b-1 的基线抬到 **7157 块/s**，但仍是 10,000 的 71.6%。缺口不是线程不够：付费块的锁内提交合计占整轮的 78.1%，而提交严格串行，所以**单提交线程的硬上限约 9164 块/s**——加预验证线程、加队列都翻不过这道墙（池利用率只有约 9%）。剩下的时间全在锁内的"其余"那一坨（**≥ 84% 的锁内时间**，与 SP0b-1 时几乎没变）。

1. **先补测量，再动代码（硬性前置）**：给基准加 `phase.*` 子步骤行——至少"链接校验里的 `AddressStore` 读"、"孤块池增删（`deleteFromQueue` 对四个队列做 `contains` + `remove`，是线性扫描）"、"难度计算与 top / 统计更新"。现在没有任何一行能把这 ≥ 84% 再拆开，不补就是在没有基线的情况下改共识路径。
2. **头号嫌疑：`AddressStore` 读**。`WriteBehindFactory` 只包 INDEX / BLOCK / TIME / ORPHANIND，**ADDRESS 是故意不包的**（它由 apply 路径写，那条路径本来就直写），而读缓存又只加在 INDEX 上，于是每个付费块的两个地址链接都是没有任何缓存的 RocksDB 读。候选解法：给 ADDRESS 加一层节点本地读缓存（写在直写路径上同步更新），或把校验需要的字段随 `PreValidated` 在锁外取（**但这会踩 P1**：`AddressStore` 是共享可变状态，锁外读到的值可能在进锁前就变了，只能当"加速通过"的提示，不能当判定）。
3. **目标**：平均每单位的锁内提交 ≤ 100 µs（目前 139.7 µs），即"其余"再砍约 34%。达不到就意味着 10,000 块/s 需要**锁内并行**（按发送方分片的多把锁），那是另一个量级的改动，SP0b-2 §1.2 已把它排除在外。
4. 与 SP0b-3 的关系：孤块池那一项两边都会碰（SP0b-3 要把 `memOrphanPool` 换成 `ChainOrphanPool`），**谁先做谁负责把那部分的 `phase.*` 行留下来**。

规格与实测：`docs/superpowers/specs/2026-09-18-xdag-chain-sp0b2-ingest-pipeline-design.md` §4.3、`docs/benchmarks/2026-09-19-l1-import-pipeline.md`。
...
**SP0b-2b（§5.2.1）插在 SP0b-2 与 SP0b-3 之间**：SP0b-2 的验收（导入 ≥ 10k 块/s）未达成，缺口在锁内"其余"；它与 SP0b-3 在孤块池上有重叠，顺序可换，但 M3 的导入门禁挂在它上面。
...
| **M3 执行** | SP1-3/4/5 + SP0b-2 完成 | 单链执行吞吐 ≥ 2k 调用/s；两模式一致；**导入 ≥ 10k 块/s —— 未达成**：SP0b-2 实测中位 **7157 块/s**（71.6%），瓶颈是单线程锁内提交里的"其余"（AddressStore 读 / 孤块池 / 难度与统计，≥ 84% 的锁内时间），单提交线程的硬上限约 9164 块/s，加线程无效。M3 这一项挂在 **SP0b-2b**（§5.2.1）上，见 `docs/benchmarks/2026-09-19-l1-import-pipeline.md` |
