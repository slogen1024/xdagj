...
| **A4** | High (design) | Fee routing (G3-T1) mints native XDAG from **unbacked EVM wei**: `evm.alloc` genesis wei has no native lock, so spending it as gas mints fresh native XDAG via the reward-pool credit. `getSupply()` doesn't account for this. This is ADR-016's explicit open item, unresolved in code. | **FAIL-SAFE + MODEL DONE (merge e43b2cd7)**: config fail-fast (`feeRewardActivationHeight` scheduled ⇒ bridge scheduled; warn on fee+alloc) shipped; target model = **transfer-from-lock** (design `2026-08-31-a4-...-design.md`). **The full transfer-from-lock / alloc-seeds-lock / getSupply consensus change is DEFERRED to activation-prep** — still blocks scheduling `feeRewardActivationHeight` on a non-throwaway net. **A4-full transfer-from-lock SHIPPED 2026-09-01** (plan docs/superpowers/plans/2026-09-01-a4-full-transfer-from-lock.md). |
...
A1's overflow guard and A3's reversal diagnostic were implemented + reviewed + merged this session (`fix(evm): guard EVM fee-credit overflow + bridge-reversal shortfall diagnostic (audit A1/A3)`). A2 was left unchanged (unreachable-by-construction). **K1 (async-drain fee-credit convergence) is now ALSO done** (merge b8c72862): both the sync and async paths credit the payload block M via a shared `creditEvmFee`, so a blob-behind node converges — the fork is closed. **A4's config fail-safe + model decision are now ALSO done** (merge e43b2cd7): fee-routing requires a scheduled bridge, target model = transfer-from-lock; the full transfer-from-lock consensus change is deferred to activation-prep. The remaining open items (**A4-full** transfer-from-lock implementation, external audit track, shared-net gas params, EVM_META recovery-path doc) gate any activation.
...
- [x] **A1** overflow guard shipped — DONE (merged with A3, 2026-08-30).
...
- [x] **A4 (fail-safe + model decided) — DONE (merge e43b2cd7)**: `validateBridgeConfig` fails fast when `feeRewardActivationHeight` is scheduled without a scheduled bridge (no deposit lock to source the fee), + warns on fee-routing with a non-empty `evm.alloc`. Target model chosen = **transfer-from-lock** (design `docs/superpowers/specs/2026-08-31-a4-fee-routing-supply-conservation-design.md`).
