...
> **下面的测法与数字一律是修复前的事实，原样保留**；紧随其后的一段是同一测量的 AFTER。
> 本条的 BEFORE/AFTER 已在此处记全，**Task 16 只需引用，不要写进基准 AFTER 表**——
> 那张表比的是 `phase.orphan.*` 与 blocks/s，而**基准夹具从不调用 `checkOrphan`**
> （`ChainL1TestBase.MockBlockchain.startCheckMain` 是空实现，夹具也不设 pow；
> 全套件里 `checkOrphan` 的测试调用方只有 `ExtraBlockTest` 与 `ChunkFloodAdversarialTest`）。
> **所以本次追加修复的出块改动动不了那张表的任何一行**——若它变了，原因在别处。
...
> **进入 8 次，建成 0 个，导入 0 个**。非分片对照臂的三个数彼此相等
> （进入 == 建成 == 导入），每建成一块 `nnoref` 降约 11。**绝对值逐次不同，测试刻意不断言**：
> 抽样从第二个 tick 起就恢复活动（`nnoref` 一动，`nblk % 61 == 0` 的前提就没了），
> 实测一次是 8 块 671 → 583、另一次是 7 块 671 → 594。对照臂要证明的是
> 「节点在非分片洪泛下确实在挖、在消化」，不是某个具体数字。
...
> ---
>
> **✅ 已解决（2026-09-23 的追加修复，缺陷一 / P1，提交 `659fdbbd`）。** 上面说的那次
> 「独立的设计变更」就是它：`dealOrphan` 的挖矿闸门改为按类别拆分，`CHUNK` 不再受它把关，
> 所以不挖矿的节点重新留得住分片块，本条诊断的 `NO_PARENT` 成因随之消失。设计见
> `docs/superpowers/specs/2026-09-23-sp0b3-chunk-pooling-without-mining-design.md`。
>
> **实测确认：** 追加修复的 Task 5 重跑了 `-Dxdag.bench.blocks=2000`，
> **BUILD SUCCESS，154.9 s，三轮 `syncPath` 全部完成**，`NO_PARENT` 不再出现；
> 并且**夹具未被触碰**（`ChainL1ImportBenchmarkTest.java` 的 md5 前后一致）。
>
> **所以 Task 16 现在要做的是：** 直接重跑基准，确认 `NO_PARENT` 已消失。
> **若它仍然失败，那是一项新发现，不是这里记录的旧成因**——不要照着上面那条链去排查。
> 无论如何**不要**为了让基准跑起来而给夹具加 pow（Task 1 的文档已经论证过那会让全部历史基线失去可比性）。
